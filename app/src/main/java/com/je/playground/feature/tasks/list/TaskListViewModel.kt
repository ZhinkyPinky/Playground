...
enum class TaskType(type: String) {
    SimpleTask("Basic Task"), ExerciseProgram("Exercise Program")
}

enum class TaskTypeV2(type: String) {
    RegularTask("Regular Task"), ExerciseTask("Exercise Task"), HabitTask("Habit Task"), ShoppingTask(
        "Shopping Task"
    )
}
...
class TaskListViewModel @Inject constructor(
    ...
    sealed class State {
        ...
        data class Ready(
            val task: Task, val subTasks: List<SubTask>,
    ...
    init {
        viewModelScope.launch {
            getActiveTasksWithSubTasks().collect {
    ...
    fun onEvent(event: TaskListEvent) {
        when (event) {
            is Delete -> deleteTask(event.taskWithSubTasks)
            is ToggleTaskCompletion -> viewModelScope.launch { toggleTaskIsCompleted(event.task) }
            is ToggleSubTaskCompletion -> viewModelScope.launch { toggleSubTaskIsCompleted(event.subTask) }
            is ToggleTaskArchived -> toggleTaskIsArchived(event.task)
        }
    }
}
